...
\begin{figure}[!htbp]
    ...
    \begin{minipage}{0.5\textwidth}
        ...
        \caption{Electricity Grid and Fossil Demand over time.}
...
\begin{itemize}
\item Non-electric fossil fuel related GHG emission intensity remains unchanged for the first 20 years \ref{fig:ghg_emissions}, then decreases as the residential sector becomes electrified and the Syngas production is started.
\item The graph \ref{fig:net_electricity_demand} shows that the net electricity demand decreases over the years due to increased self-sufficiency of the residential installations.
\item In the transportation sector, once hydrogen is introduced the GHG emission intensity is reduced and remains constant as variations in transportation demand were not simulated \ref{fig:ghg_emissions}.
\item The local mix GHG emission intensity changes with SMR introduction and increases slightly as PV share grows, due to its higher GHG intensity compared to nuclear. This PV share variation is driven by an increase in residential PV installations, boosting solar share \ref{fig:ghg_emissions}.
\item The graph \ref{fig:ghg_emissions} indicates that the impact on Lombardy's GHG intensity is minimal, considering the small production of the plant compared to the regional demand.
\end{itemize}

\subsection{Land Use}
One of the key advantages of the proposed project is its minimal land use impact. The compact scale of the facility, combined with its placement in an area already approved for nuclear activities, ensures efficient use of space. The site is located within the JRC complex, which already hosts research reactors (ESSOR and Ispra-1) and associated nuclear infrastructure. Notably, a containment building is available on-site and might be deemed suitable to house at least one of the two planned SMRs, greatly reducing the need for new construction and environmental disturbance.